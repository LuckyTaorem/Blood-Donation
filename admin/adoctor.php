...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM jdoctor ORDER BY id ASC");
    $stmt->execute();
  ...
  <?php if (count($result) > 0) { 
      foreach ($result as $row) { ?>
          <tr>
              ...
              <td><a href='logic.php?deldoctor=<?= $row['id'] ?>' type='button' class='btn btn-danger'>Delete</a></td>
          </tr>
      <?php } 
  } else { ?>
      <tr>
