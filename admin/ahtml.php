...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM jhtml ORDER BY id ASC");
    $stmt->execute();
...
<table class="table table-dark">
    ...
    <tr>
        <td colspan="12" class="text-center">
            <h4>Web Developer Job Applicant</h4>
        </td>
    </tr>
    <tr>
        <th scope="col">Id</th>
  ...
  <?php if (count($result) > 0) {
        foreach ($result as $row) { ?>
            <tr>
                ...
                <td><a href='logic.php?delhtml=<?= $row['id'] ?>' type='button' class='btn btn-danger'>Delete</a></td>
            </tr>
        <?php } 
    } else { ?>
        <tr>
